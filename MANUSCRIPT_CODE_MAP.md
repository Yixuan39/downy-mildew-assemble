...
| Table 1 | Assembly statistics for the mitochondrial-filtered nuclear assemblies | `workflow/05-assembly-qc/{split-contigs.sh,qc-final-assemblies.sh}` | `analysis/final-assembly.Rmd` | - (computed on the cluster QC output, not committed) | `figures/final-assembly-top-contigs.* (companion)` |
...
| Supp. Fig. S3 | Read-level GC content vs. k-mer coverage (blob plot), checking whether target Oomycota reads separate from contamination | `workflow/01-read-filtering-screening/coverage-gc/{kat-seqkit-coverage-gc.sh,summarize-coverage-gc.R}` | `analysis/read-distribution.Rmd` | `data/blobplot-read-coverage-gc-subsample.tsv; data/blobplot-category-summary-by-isolate.tsv` | `figures/blobplot-coverage-gc.*` |
...
| Supp. Table S3 | RepeatMasker summary for P. effusa UA202013* | `workflow/07-repeatmask-gene-prediction/hard-mask-*.sh` | - | `data/repeatmasker/Peronospora_effusa_UA202013_star_repeatmasker_summary.tsv` | - |
| Results text - SC1982 gap and contig split | Read support across the Pcub-SC1982_002 gap and contig tail, and the split of that contig into 002a/002b | `workflow/05-assembly-qc/{check-sc1982-gap-tail-coverage.sh,summarize-sc1982-14-gene-support.R,split-contigs.sh}` | - | `data/sc1982_gap_tail_coverage/; data/sc1982_submission_split/` | - (only the coverage/support evidence tables are committed) |
