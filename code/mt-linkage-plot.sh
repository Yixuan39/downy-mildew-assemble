#!/usr/bin/env bash
# Linear synteny/linkage plot of the 14 oomycete mitochondrial genomes via gbdraw.
# Row order mirrors the nuclear synteny figure (analysis/synteny-analysis-contigs.Rmd),
# whose rows come from the GENESPACE SpeciesTree_rooted.txt tip order.
# Usage: mt-linkage-plot.sh <multi-record.gb> [outdir]
set -euo pipefail
