# read_report2(): parse a Kraken2/Centrifuge-style .kreport into a data.frame with one row
# per taxon and, when add_taxRank_columns = TRUE, one column per kept taxRank giving that
# row's ancestor at that rank (used by read-distribution.Rmd to pull the phylum-level (P)
# ancestor of each report row for the taxonomy-composition and blob-plot categorization).
#
# Vendored from pavian (https://github.com/fbreitwieser/pavian, GPL-3), commit 14fbaf4,
# R/datainput-read_report.R, unmodified apart from the dmessage() stub below. pavian itself
# is a Shiny app; installing it pulls in shiny/httpuv/DT/sankeyD3/... which failed to build
# from source on the NCSU BRC cluster's R conda env (missing compiled deps for later/promises/
# httpuv). read_report2() only needs base R + utils::read.table, so it is vendored directly
# instead of carrying the whole app as a dependency just to parse a report file.
dmessage <- function(...) invisible(NULL)
