...
| `UA202013/kraken2-pluspfp.sh` | Same Kraken2 PlusPFP classification for the public P. effusa reads, which live in their own directory and are a single file (no array). | NCSU BRC, SLURM, 24 cores / 220 GB |
| `UA202013/run-hifiadapterfilt.sh` | Same filtlong + HiFiAdapterFilt step for the public P. effusa reads (single file, no array). | NCSU BRC, SLURM, 32 cores |
