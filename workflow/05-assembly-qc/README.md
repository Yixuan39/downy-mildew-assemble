...
| `qc-other-genomes.sh` | Same quality workflow over the published downy mildew genomes, so the new assemblies can be compared on identical metrics. | local or cluster; needs Nextflow with the docker profile |
...
`qc-final-assemblies.sh` and `qc-other-genomes.sh` need Nextflow with the `docker` profile and a
