...
# Inputs  : ${PROJECT_DATA}/results/assembly-qc/nuclear/Pseudoperonospora_cubensis_SC1982.fasta.gz
#           ${PROJECT_DATA}/results/read-filtering-screening/reads/focal/Quesada_SQIIe_SC1982.fastq.gz
...
# Usage   : bash workflow/05-assembly-qc/check-sc1982-gap-tail-coverage.sh
set -euo pipefail
source "${REPO_ROOT:-${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}}/workflow/paths.sh"

REFERENCE="${REFERENCE:?Set REFERENCE to the pre-split SC1982 FASTA}"
READS="${PROJECT_DATA}/results/read-filtering-screening/reads/focal/Quesada_SQIIe_SC1982.fastq.gz"
CONTIG="Pcub-SC1982_002"
OUTDIR="$REPO_ROOT/data/sc1982_gap_tail_coverage"
THREADS=16
...
echo "Created ${BAM}"
echo "Run: Rscript workflow/05-assembly-qc/summarize-sc1982-14-gene-support.R"
